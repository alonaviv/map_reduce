...
TEST_CASE("execute_map functionality tests, single thread")
{
    ...
    std::atomic<size_t> vector_index(0);
    ...
    SECTION("use execute_map with a lot of pairs, running a few fetch loops")
    {
        _emplace_pairs(in_items, 20);
        auto res = execute_map(map_reduce_base, in_items, vector_index, 5);
        _test_pair_results(res, 20);
        REQUIRE(vector_index.load() == 20);
    }
}